...
class Usuarios extends BaseController
{
    ...
    public function licencia_1()
    {
        return view('licencia_1');
    }

    public function licencia_2()
    {
        return view('licencia_2');
    }

    public function acepta_licencia_1()
    {
        if ($this->request->getMethod() == "post") {

            $session = session();
            $idUsuario = $session->id_administrador;
            $this->administradores->update($idUsuario, ['licencia' => '1']);

            return redirect()->to(base_url() . '/Usuarios/valida');
        }
    }

    public function acepta_licencia_2()
    {
        if ($this->request->getMethod() == "post") {
            ...
            $this->profesionales->update($idUsuario, ['licencia' => '1']);
    ...
    public function valida()
    {
        if ($this->request->getMethod() == "post") {
            ...
            if ($datosUsuario != null && password_verify($password, $datosUsuario['password']) == true && $datosUsuario['estado'] == '1' && $datosUsuario['login'] == $usuario) {
                ...
                if (($session->licencia) == "0") {
                    return redirect()->to(base_url() . '/Usuarios/licencia_1');
                } else {
                    if (($session->reset) == "0") {
                ...
                if ($datosUsuarioProf != null && password_verify($password, $datosUsuarioProf['password']) == true && $datosUsuarioProf['estado'] == '1' && $datosUsuarioProf['cedula'] == $usuario) {
                    ...
                    if (($session->licencia) == "0") {
                        return redirect()->to(base_url() . '/Usuarios/licencia_2');
                    } else {
                        if (($session->reset) == "0") {
                            return redirect()->to(base_url() . '/Usuarios');
                        } else {
